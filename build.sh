...
tmp_python_ssl="/prepare/tmp_python_ssl"


function display_usage() {
 log_info "$0 [-c] [-h] [-i] [-m] [-s] <path/to/tcollector/root>"
}
...
function install_pip() {
      ...
      log_info 'set up pip ...'
      if [[ ! -f ${workspace_folder}/pip-9.0.1.tar.gz ]]; then
        log_info 'download pip-9.0.1 tarball '
        wget --directory-prefix="${workspace_folder}" https://download.cloudwiz.cn/package/pip-9.0.1.tar.gz#md5=35f01da33009719497f01a4ba69d63c9
        abort_if_failed 'failed to download download pip-9.0.1 tarball'
      fi
      tar -xzf "${workspace_folder}"/pip-9.0.1.tar.gz -C "${workspace_folder}"
      abort_if_failed 'failed to extract setuptools tarball'

      pushd "${workspace_folder}"/pip-9.0.1
      "${altenv_bin_folder}"/python setup.py install --prefix="${altenv_folder}"
      abort_if_failed 'failed to install setuptools'
      popd
      log_info 'finish setting up pip'
}

function install_python_ssl() {
...
function py_complite_so() {
   ${altenv_bin_folder}/python py_complite_so.py
}
...
if [[ ! "$skip" = true ]]; then
 ...
 "${altenv_bin_folder}/python" -m pip  install -r requirement.txt -i https://pypi.douban.com/simple
  ...
  fb_tarball="filebeat-5.6.5-linux-${bitness}.tar.gz"
...
cp ${collector_source_path}/agent_main.py ${agent_collector_folder}/agent_main.py
cp ${collector_source_path}/runner.py ${agent_collector_folder}/runner.py
abort_if_failed 'failed to copy runner.py'


cp ${collector_source_path}/agent_main.conf ${agent_collector_folder}/agent_main.conf
abort_if_failed 'failed to copy agent_main.conf'
echo -e "version=$(git describe --abbrev=0 --tags)" >> ${agent_collector_folder}/agent_main.conf
echo -e "commit=$(git log --format="%H" -n 1 | head -c 6)" >> ${agent_collector_folder}/agent_main.conf
...
if [ "$COMPILE" == "compile" ];then
  log_info 'start py compiling to so...'
  py_complite_so
  abort_if_failed 'failed to py_complite_so'
fi
log_info 'start unistall  cython '
"${altenv_bin_folder}/python" -m pip  uninstall Cython -y
abort_if_failed 'unistall  cython'
...
mkdir -p ${agent_install_folder}/uagent
abort_if_failed "failed to create ${agent_install_folder}/uagent"
yes | cp -f "${collector_source_path}/uagent/daemon.py" "${agent_install_folder}/uagent/"
abort_if_failed "failed to copy ${basedir}/uagent/daemon.py to ${agent_install_folder}/uagent/"
yes | cp -f "${collector_source_path}/uagent/run-uagent.sh" "${agent_install_folder}/uagent/"
abort_if_failed "failed to copy ${basedir}/uagent/run-uagent.sh to ${agent_install_folder}/uagent/"
yes | cp -f "${collector_source_path}/uagent/uagent.conf" "${agent_install_folder}/uagent/"
abort_if_failed "failed to copy ${basedir}/uagent/uagent.conf to ${agent_install_folder}/uagent/"
yes | cp -f "${collector_source_path}/uagent/uagent.py" "${agent_install_folder}/uagent/"
abort_if_failed "failed to copy ${basedir}/uagent/uagent.py to ${agent_install_folder}/uagent/"
...
if [[ "$major" = true ]]; then
  ./gen_version.py --major ${agent_install_folder}/version.json
elif [[ "$minor" = true ]]; then
  ./gen_version.py --minor ${agent_install_folder}/version.json
else
  ./gen_version.py --patch ${agent_install_folder}/version.json
fi
abort_if_failed "failed to update ${agent_install_folder}/version.json"
